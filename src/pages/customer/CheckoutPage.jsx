...
const StripeCardForm = forwardRef(function StripeCardForm({ total }, ref) {
...
export default function CheckoutPage() {
  ...
  async function handleConfirm() {
    ...
    // Pago con tarjeta → cobrar con Stripe antes de crear el pedido
    if (payMethod === 'card') {
      try {
        await stripeFormRef.current.pay()
      } catch (err) {
          ...
          <Section title="Entrega" icon={Package}>
            <div className="grid grid-cols-2 gap-2.5">
              {DELIVERY_OPTIONS.map(({ id, label, Icon }) => (
                <button
                  key={id} type="button" onClick={() => setDelivery(id)}
            ...
            {payMethod === 'card' && (
              ...
              >
                <div className="rounded-2xl border border-bk-border bg-bk-card p-4">
                  <StripeCardForm ref={stripeFormRef} total={total} />
                </div>
              </Elements>
            )}
